...
import sys
sys.path.insert(0, '/home/rasppi3/Downloads/robotics-level-4-main/earthrover')
import util as ut
ut.init_gpio()
...
model_dir = '/home/rasppi3/Downloads/robotics-level-4-main/all_models'
    ...
    if(abs(x_deviation)<tolerance and abs(y_deviation)<tolerance):
        ...
        if (abs(x_deviation)>abs(y_deviation)):
            ...
            if(y_deviation>=tolerance):
                cmd="Tien len"
                delay1=get_delay(y_deviation,'f')*2.5
            ...
            if(y_deviation<=-1*tolerance):
                cmd="Lui xuong"
                delay1=get_delay(y_deviation,'b')*2.5
...
def draw_overlays(cv2_im, objs, labels, arr_dur, arr_track_data):
    ...
    text_dur = 'Camera: {}ms   XL Khung: {}ms   Khac: {}ms'.format(cam,inference,other)
    ...
    str_sp='Toc do: {}%'.format(round(delay1/(0.1)*100,1))
